...
async def on_command_error(ctx, error):
	...
	if isinstance(error, commands.MissingRequiredArgument):
		embed.add_field(name='Missing an argument!',
...
async def help_command(ctx, command=None):
	...
	if command != None:
		...
		if command == 'chart':
			embed.add_field(
			    ...
			    value=
			    'Generate a chart for given symbol in given duration. Durations: M, W, D, 15, 5, 3 are for the monthly, weekly, daily, 15, 5, and 3 minute.'
			)

		elif command == 'help':
			embed.add_field(name='Help Command', value='Help with the help?')

		elif command == 'quote':
			embed.add_field(name='!quote <symbol>',
	...
	# List basic help info
		...
		embed.add_field(name='!chart <symbol> <duration>',
...
async def sheesh(ctx):
	desc = [
	    '(っ・ω・）っ≡≡≡≡≡≡.*･☆', '[^._.^]ﾉ', '( ˘•ω• ˘ )', '(´⊙ω⊙` )ᵒᵐᵍᵎᵎᵎ',
	    '┗(･ω･; )┛', '@(´ ･ｪ･ `)@'
	]
	desc = random.choice(desc)
	embed = discord.Embed(title="SHEEeeeeEEESH!!!!!",
	                      description=desc,
...
async def hot(ctx):
	desc = ['heads', 'tails']
	desc = random.choice(desc)
	embed = discord.Embed(title=desc, color=discord.Color.greyple())
	await ctx.send(embed=embed)
